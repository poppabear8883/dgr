      ...
      <div class="container">
        <div class="row">
          ...
          <div class="col-xs-12 col-md-6">
            <div @click="update(editing_id)" class="btn-lrg form-btn">Save</div>
          </div>
          <div class="col-xs-12 col-md-6">
            <i @click="destroy(editing_id)" class="fa fa-trash fa-2x color-red pull-right"></i>
          </div>
        </div>
      </div>
    </modal>
    ...
    methods: {
      ...
      destroy(id) {
        ...
        axios.delete(`/api/galleries/${id}`)
          .then((response) => {
            this.success = `Successfully Deleted gallery with id of ${id}`;
            this.reload();
          }).catch((error) => {
          this.errors.push(error.response.data.message);
        });
